...
/*	var request = this.oauth.post(url + '?' + querystring.stringify(params),
	...
	var request = this.oauth.post(url,
		this.options.access_token_key,
		this.options.access_token_secret, params);
 ...
 */
Twitter.prototype.cookie = function(req, res) {
	// Fetch the cookie
	var twauth = null;
	var cookies = new Cookies(req, res, this.options.keygrip);
	var cooks = cookies.get(this.options.cookie, { signed: true});
	try {
		twauth = JSON.parse(cooks);
	} catch (error) {
		// ignore
	}
	return twauth;
...
/*
 * clears the current users cookie (signout)
 */
Twitter.prototype.clearCookie = function(req, res) {
	// Fetch the cookie
	var cookies = new Cookies(req, res, this.options.keygrip);
	cookies.set(this.options.cookie, null, this.options.cookie_options);
}
...
Twitter.prototype.login = function(mount, success) {
	...
	return function handle(req, res, next) {
		...
		// Fetch the cookie
		var twauth = self.cookie(req, res);
		...
		// Returning from Twitter with oauth_token
			...
			function(error, access_token_key, access_token_secret, params) {
				...
				if ( error ) {
					...
					var cookies = new Cookies(req, res, self.options.keygrip);
		...
		// Begin OAuth transaction if we have no cookie or access_token_secret
			...
			function(error, oauth_token, oauth_token_secret, oauth_authorize_url, params) {
				if ( error ) {
					...
					res.writeHead(302, {
						'Location': self.options.authenticate_url + '?'
